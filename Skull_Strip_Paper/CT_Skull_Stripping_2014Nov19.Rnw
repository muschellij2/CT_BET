...
\subsubsection{Intraclass Correlation Estimate}
For the intraclass correlation estimate of brain volume, we collected \Sexpr{nscans_total} scans from \Sexpr{npt_total} patients.  Of these, we excluded \Sexpr{n_crani_problem} due to craniotomy and \Sexpr{n_gantry_problem} due to the gantry tilt correction forcing areas of the brain outside the field of view.  
